    ...
    for date in sorted(test_df["date"].unique()):
        ...
        for order_book_id, pos in list(positions.items()):
            ...
            if profit >= 0.50:
                ...
                continue
            if profit >= 0.30 and not pos.get("trimmed_30", False):
                pos["weight"] = pos["base_weight"] * 0.5
                pos["trimmed_30"] = True
            if market_drawdown <= 0.06:
                del positions[order_book_id]
...
def main():
    ...
    out = OUTPUT_DIR / "hybrid_market_drawdown10_resilient_momentum20_ml_top20_staged_tp_summary.json"
