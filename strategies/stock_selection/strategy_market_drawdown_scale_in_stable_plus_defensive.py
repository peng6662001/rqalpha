...
def init(context):
    ...
    context.bear_ma_period = 60
    context.bear_market_scale = 0.75
...
def _bear_market_cap(context, benchmark_price):
    history = history_bars(context.benchmark, context.bear_ma_period, "1d", "close")
    if history is None or len(history) < context.bear_ma_period:
        return 1.0
    moving_average = history.mean()
    if moving_average <= 0:
        return 1.0
    if benchmark_price < moving_average:
        return context.bear_market_scale
    return 1.0
...
def handle_bar(context, bar_dict):
    ...
    bear_cap = _bear_market_cap(context, benchmark_price)
    if bear_cap < 1.0:
        target_percent = min(target_percent, bear_cap)
        plot("bear_cap", bear_cap)
