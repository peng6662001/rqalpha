...
def prepare_dataset():
    ...
    dataset["stock_drawdown_120"] = -dataset["dd_60"]  # fallback placeholder for readability
    return dataset, feature_cols, benchmark


def add_signal_columns(dataset):
    ds = dataset.copy()
    ds["stock_drawdown_120"] = 1.0 - (1.0 + ds["dd_60"])  # overwritten below when possible
    ds["resilient_ok"] = (
        (ds["close"] >= 5.0)
        & (ds["close"] <= 120.0)
...
def compute_true_120_drawdown(dataset):
    # Use existing 120-lookback close series per stock from built features proxy:
    # dd_60 is insufficient, so derive with rolling max on close inside grouped frame.
    ds = dataset.sort_values(["order_book_id", "date"]).copy()
...
def run_hybrid_year(dataset, feature_cols, benchmark_df, year):
    ...
    test_df["ml_bullish"] = test_df["pred"] > 0
    test_df["hybrid_score"] = test_df["ret_20"] + 0.35 * test_df["ret_10"] + 0.10 * test_df["ret_60"] - 0.20 * test_df["stock_drawdown_120"]
    ...
    for date in all_dates:
        ...
        # exit on take profit / market reset
        exited = set()
        for order_book_id, pos in list(positions.items()):
            ...
            if profit >= 0.50 or market_drawdown <= 0.06:
                exited.add(order_book_id)
                del positions[order_book_id]

        if market_drawdown >= 0.10:
            candidates = day_rows[day_rows["resilient_ok"] & day_rows["ml_bullish"]].copy()
            ...
            if not selected.empty:
                top_snapshots.append(
                    {
                        "date": str(date.date()),
                        "top": selected[["order_book_id", "symbol", "pred", "ret_20", "stock_drawdown_120"]]
                        .head(3)
    ...
    metrics["year"] = year
    return metrics, top_snapshots[:5]
...
def main():
    ...
    dataset = compute_true_120_drawdown(dataset)

    rows = []
    snapshots = {}
    for year in range(2020, 2026):
        result = run_hybrid_year(dataset, feature_cols, benchmark_df, year)
        if result is None:
            continue
        metrics, shots = result
        rows.append(metrics)
        snapshots[str(year)] = shots
    ...
    out = OUTPUT_DIR / "hybrid_market_drawdown10_resilient_momentum20_ml_filter_summary.json"
