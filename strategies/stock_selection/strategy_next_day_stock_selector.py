...
def _is_tradeable_candidate(context, order_book_id, symbol, metrics):
...
def after_trading(context):
    ...
    for order_book_id in context.stock_pool:
        ...
        item.update(metrics)
        item["tradeable"] = _is_tradeable_candidate(context, order_book_id, symbol, metrics)
        results.append(item)
    ...
    output_path = Path.cwd() / ".temp" / f"next_day_selection_{selection_date}.json"
    output_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    logger.info(f"next-day selector wrote: {output_path}")
