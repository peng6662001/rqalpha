...
def run_hybrid_year(dataset, feature_cols, benchmark_df, year):
    ...
    for date in all_dates:
        ...
        desired_exposure = target_exposure(market_drawdown)
        ...
        for oid, pos in positions.items():
            if pos.get("weight", 0.0) <= 0:
                continue
            row = day_rows[day_rows["order_book_id"] == oid]
            if row.empty:
                continue
            realized += float(row["next_ret_1"].iloc[0]) * float(pos["weight"])
...
def main():
    ...
    out = OUTPUT_DIR / "hybrid_market_drawdown10_12_15_resilient_momentum20_ml_filter_summary.json"
