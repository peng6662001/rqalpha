...
def _train_year_if_needed(context, year):
    ...
    if train_df.empty or test_df.empty:
        ...
        logger.info(f"top1-trend-filter: skipped training for {year} due to empty train/test window")
        return
    ...
    logger.info(f"top1-trend-filter: trained {context.model_kind} model for {year} with {len(signal_dates)} signal dates")
...
def _passes_trend_filter(row):
    return (
        float(row["ret_20"]) > 0.0
        and float(row["ret_60"]) > 0.0
        and float(row["ma_gap_20"]) > 0.0
    )
...
def _pick_for_selection_date(context, year, selection_date, bar_dict):
    ...
    for _, row in group.iterrows():
        if not _passes_trend_filter(row):
            continue
...
def open_auction(context, bar_dict):
    ...
    if row is None:
        logger.info(f"top1-trend-filter: {selection_date.date().isoformat()} -> cash")
    else:
        logger.info(
            f"top1-trend-filter: {selection_date.date().isoformat()} -> "
