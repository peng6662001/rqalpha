...
def _resilient_momentum_metrics(context, order_book_id):
    ...
    ret_10 = float(prices[-1] / prices[-10] - 1.0) if len(prices) >= 10 else ret_20
    ret_60 = float(prices[-1] / prices[-60] - 1.0) if len(prices) >= 60 else ret_20
    if ret_20 < context.min_ret_20:
        return None

    score = ret_20 + 0.35 * ret_10 + 0.10 * ret_60 - 0.20 * stock_drawdown
    return {
        "score": float(score),
...
def _rank_resilient_momentum_stocks(context):
    ranked = []
    for order_book_id in context.stock_pool:
        ...
        metrics = _resilient_momentum_metrics(context, order_book_id)
        if metrics is None:
            continue
...
def handle_bar(context, bar_dict):
    ...
    ranked = _rank_resilient_momentum_stocks(context)
    selected = []
    ...
    if selected:
        logger.info(
            f"大盘回撤超过10%，买入抗跌且近20日最强股票 {selected[0]} 等{len(selected)}只 "
