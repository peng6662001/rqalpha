...
def init(context):
    ...
    context.min_rebound_ret_10 = 0.03
    context.min_rebound_ret_5 = 0.01
    context.min_turnover_ratio_5_20 = 0.20
    context.last_pool_key = None
    update_universe([context.benchmark])


def _history_field(order_book_id, bars, field):
    data = history_bars(order_book_id, bars, "1d", field)
...
def _avg_turnover(order_book_id, bars):
    turnover = _history_field(order_book_id, bars, "total_turnover")
    if turnover is None:
        return None
...
def _candidate_score(context, order_book_id):
    ...
    if len(prices) < 20:
        return None
    ret_10 = float(prices[-1] / prices[-10] - 1.0)
    ret_5 = float(prices[-1] / prices[-5] - 1.0)
    ret_20 = float(prices[-1] / prices[-20] - 1.0)
    if ret_10 < context.min_rebound_ret_10 or ret_5 < context.min_rebound_ret_5:
        return None
    if ret_5 < ret_20:
        return None

    turnover_5 = _avg_turnover(order_book_id, 5)
    if turnover_5 is None:
        return None
    turnover_ratio = turnover_5 / avg_turnover_20 - 1.0
    if turnover_ratio < context.min_turnover_ratio_5_20:
        return None

    score = drawdown + 0.25 * ret_10 + 0.20 * ret_5 + 0.15 * turnover_ratio
    return {
        "score": float(score),
