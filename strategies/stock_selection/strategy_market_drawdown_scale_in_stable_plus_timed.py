...
def init(context):
    ...
    context.max_hold_profit_floor = 0.08
    context.time_stop_trim = 0.20
...
def handle_bar(context, bar_dict):
    ...
    if profit is not None:
        ...
        if holding_days >= context.max_hold_days and profit < context.max_hold_profit_floor:
            target_percent = min(target_percent, context.time_stop_trim)
            logger.info(
                f"time stop trim: holding_days={holding_days} profit={profit:.4f} target={target_percent:.2f}"
            )
