...
def init(context):
    ...
    context.trailing_activation_profit = 0.22
    context.trailing_stop_drawdown = 0.09
    context.trailing_exit_drawdown = 0.13
    context.max_hold_days = 80
    context.max_hold_profit_floor = 0.10
    context.time_stop_trim = 0.25
...
def _time_stop_triggered(context, price, profit):
    history = history_bars(context.target, context.time_stop_ma_period, "1d", "close")
    if history is None or len(history) < context.time_stop_ma_period:
        return False
    moving_average = history.mean()
    if moving_average <= 0 or profit >= context.max_hold_profit_floor:
        return False
...
def handle_bar(context, bar_dict):
    ...
    if position.quantity > 0 and context.last_position_quantity <= 0:
    ...
    if context.wait_for_reset:
        if drawdown <= context.reset_drawdown:
            ...
            context.last_position_quantity = position.quantity
            return
    ...
    if profit is not None:
        ...
        if (
            holding_days >= context.max_hold_days
            and _time_stop_triggered(context, target_price, profit)
        ):
            target_percent = min(target_percent, context.time_stop_trim)
            logger.info(
                f"timed trend trim: holding_days={holding_days} profit={profit:.4f} target={target_percent:.2f}"
            )
